			...
			<div class="grid grid-cols-2 items-center m-0 border border-gray-200 rounded-lg shadow text-white">
				...
				<div class="flex flex-col justify-between p-4 leading-normal">
					...
					<div class="flex items-center justify-between text-gray-500">
						<div class="flex items-center space-x-2">
							<button class="flex justify-center items-center gap-2 px-2 rounded-full p-1">
                                ...
                                <form action="{{ url('like/'.$post->id) }}" method="POST">
                                    ...
                                    <button type="submit">
								<svg class="w-5 h-5 @auth{{$post->likers->find(auth()->user()->id) == true ? "text-red-500" : "" }}@endauth hover:text-red-500 fill-current" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                        ...
                        <button class="flex  justify-center items-center gap-2 px-2 rounded-full p-1">
                            <i class="fa-solid fa-bookmark @auth{{$post->bookmarkers->find(auth()->user()->id) == true ? "text-yellow-300" : "" }}@endauth hover:text-yellow-300"></i>
                        </button>
                    </form>
					</div>
					<hr class="mt-2 mb-2">
