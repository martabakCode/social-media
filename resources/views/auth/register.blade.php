...
<div class="font-[sans-serif] text-white bg-black">
	<div class="min-h-screen flex fle-col items-center justify-center py-6 px-4">
		<div class="grid md:grid-cols-2 items-center gap-10 max-w-6xl w-full">
			...
			<form class="space-y-6 max-w-md md:ml-auto max-md:mx-auto w-full" method="POST" action="{{ route('register') }}"> @csrf <h3 class="text-3xl font-extrabold mb-8 max-md:text-center"> Register </h3>
				<!-- Validation Errors --> @if ($errors->any()) <div class="mb-4">
				...
				</div> @endif <div class="grid sm:grid-cols-2 gap-2">
					<div>
					...
					<div>
						<input name="name" value="{{ old('name') }}" type="text" autocomplete="name" required class="bg-gray-100 text-black w-full text-sm px-4 py-3.5 rounded-md outline-cyan-600" placeholder="Name" required autofocus />
					</div>
				</div>
				<div>
					<input name="email" value="{{ old('email') }}" type="email" autocomplete="email" required class="bg-gray-100 text-black w-full text-sm px-4 py-3.5 rounded-md outline-cyan-600" placeholder="Email" required autofocus />
				</div>
				<div>
				...
				<div>
					<input name="password_confirmation" type="password" autocomplete="current-password" required class="bg-gray-100 text-black w-full text-sm px-4 py-3.5 rounded-md outline-cyan-600" placeholder="Confirmation Password" required autocomplete="current-password" />
				</div>
				<div class="!mt-10">
					<button type="submit" class="w-full shadow-xl py-2.5 px-4 text-sm font-semibold rounded text-white bg-cyan-600 hover:bg-cyan-700 focus:outline-none"> Register </button>
				</div>
